...
{
	if (region.aD.af === region.aP.af)
	{
		return 'on line ' + region.aD.af;
	}
	return 'on lines ' + region.aD.af + ' through ' + region.aP.af;
}
...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		try {
			xhr.open(request.bZ, request.ct, true);
		} catch (e) {
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {aR: fragment, aX: host, a6: path, a8: port_, bb: protocol, bc: query};
	});
...
var $author$project$Main$decodeSolveResponse = function (response) {
	switch (response.$) {
		case 4:
			var body = response.b;
			return A2(
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{bz: false, bD: r.bD, bN: r.bN, aV: r.aV, bZ: r.bZ, cp: r.cp, br: r.br, ct: r.ct}));
};
var $elm$http$Http$post = function (r) {
	return $elm$http$Http$request(
		{bD: r.bD, bN: r.bN, aV: _List_Nil, bZ: 'POST', cp: $elm$core$Maybe$Nothing, br: $elm$core$Maybe$Nothing, ct: r.ct});
};
var $author$project$Main$Failed = function (a) {
	return {$: 7, a: a};
};
var $author$project$Main$Solved = F2(
	function (a, b) {
		return {$: 6, a: a, b: b};
	});
var $author$project$Main$responseToMsg = function (result) {
	if (!result.$) {
		var _v1 = result.a;
		var count = _v1.a;
		var possibilities = _v1.b;
		return A2($author$project$Main$Solved, count, possibilities);
	} else {
		var error = result.a;
		return $author$project$Main$Failed(error);
	}
};
var $elm$core$String$concat = function (strings) {
	return A2($elm$core$String$join, '', strings);
};
var $author$project$Main$feedbackLabel = function (feedback) {
	switch (feedback) {
		case 0:
			return 'B';
		case 1:
...
var $author$project$Main$guessEncoder = function (guess) {
		...
		_List_fromArray(
			[
				...
				A2(
					...
					$author$project$Main$guessEncoder,
...
var $author$project$Main$solve = function (model) {
	return $elm$http$Http$post(
		{
			bD: $elm$http$Http$jsonBody(
				$author$project$Main$solveRequestEncoder(model)),
			bN: A2($elm$http$Http$expectStringResponse, $author$project$Main$responseToMsg, $author$project$Main$decodeSolveResponse),
			ct: '/api/solve'
		});
};
var $author$project$Main$updateFeedbackAt = F3(
	function (index, transform, feedback) {
		return A2(
			$elm$core$List$indexedMap,
			F2(
				function (itemIndex, item) {
					return _Utils_eq(itemIndex, index) ? transform(item) : item;
				}),
			feedback);
	});
var $author$project$Main$updateGuess = F3(
	function (id, transform, guesses) {
		return A2(
			$elm$core$List$map,
			function (guess) {
		...
		switch (msg.$) {
					...
					_Utils_update(
						model,
						{
							q: _Utils_ap(
								...
								_List_fromArray(
									[
										$author$project$Main$initialGuess(model.ag)
									])),
							ag: model.ag + 1
						}),
					$elm$core$Platform$Cmd$none);
			...
			case 6:
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{am: count, L: '', O: false, ap: possibilities}),
					$elm$core$Platform$Cmd$none);
			case 7:
				var error = msg.a;
				return _Utils_Tuple2(
...
var $author$project$Main$AddGuess = {$: 2};
var $author$project$Main$CustomCandidatesChanged = function (a) {
	return {$: 4, a: a};
};
var $author$project$Main$Reset = {$: 8};
var $author$project$Main$Solve = {$: 5};
...
var $author$project$Main$RemoveGuess = function (a) {
	return {$: 3, a: a};
};
var $author$project$Main$FeedbackChanged = F2(
	function (a, b) {
		return {$: 1, a: a, b: b};
	});
...
var $author$project$Main$feedbackButton = F3(
	function (id, index, feedback) {
		return A2(
			...
			_List_fromArray(
				[
					...
					'aria-label',
					'Feedback position ' + $elm$core$String$fromInt(index + 1)),
					$elm$html$Html$Events$onClick(
					A2($author$project$Main$FeedbackChanged, id, index))
				]),
			_List_fromArray(
				[
					$elm$html$Html$text(
...
var $author$project$Main$guessView = F2(
	function (model, guess) {
		return A2(
			$elm$html$Html$section,
			_List_fromArray(
				[
					$elm$html$Html$Attributes$class('guess-row')
				]),
			_List_fromArray(
				[
					A2(
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('feedback-grid')
						]),
					A2(
						$elm$core$List$indexedMap,
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$class('guess-list')
							]),
						A2(
							$elm$core$List$map,
							function (guess) {
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$class('word-grid')
							]),
						A2($elm$core$List$map, $author$project$Main$wordView, model.ap))
					]))
			]));
};
